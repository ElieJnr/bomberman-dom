...
function destroyBrick(row, col, playername) {
  const tileType = tileMap.getTile(col, row);
  const tileDiv = document.getElementById(
    `${mapClass.BRITTLE_BRICK_CLASS}-${row}-${col}`
  );
  ...
  if (tileType === tileMap.tileTypes.BRICK_WITH_POWERUP) {
    ...
    tileMap.bricksWithPowerup = tileMap.bricksWithPowerup.filter(
      (brick) => brick.row !== row || brick.col !== col
    );
  ...
  directions.forEach((direction) => {
    ...
    if (
      newRow >= 0 &&
      newRow < tileMap.map.length &&
      newCol >= 0 &&
      newCol < tileMap.map[0].length
    ) {
      ...
      if (adjacentTileType === tileMap.tileTypes.BRICK) {
        ...
        const adjacentTileDiv = document.getElementById(
          `${mapClass.BRITTLE_BRICK_CLASS}-${newRow}-${newCol}`
        );
        if (adjacentTileDiv) {
      ...
      if (
        Math.abs(direction.rowOffset) === 2 ||
        Math.abs(direction.colOffset) === 2
      ) {
        for (let offset = 1; offset <= 2; offset++) {
          ...
          if (
            extendedRow >= 0 &&
            extendedRow < tileMap.map.length &&
            extendedCol >= 0 &&
            extendedCol < tileMap.map[0].length
          ) {
            ...
            if (extendedTileType === tileMap.tileTypes.BRICK) {
              ...
              const extendedTileDiv = document.getElementById(
                `${mapClass.BRITTLE_BRICK_CLASS}-${extendedRow}-${extendedCol}`
              );
              if (extendedTileDiv) {
  ...
  for (const playername in playerPositions) {
    ...
    if (
      (parseInt(playerRow) === row + 1 && parseInt(playerCol) === newCol) ||
